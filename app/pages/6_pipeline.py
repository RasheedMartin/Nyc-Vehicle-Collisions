...
from theme import CHART_BG, TEXT, SUBTEXT, ACCENT
...
try:
    trained_at = datetime.fromisoformat(train_meta["trained_at"]).strftime("%b %d, %Y  %I:%M %p")
except Exception:
...
for i, (title, subtitle, color) in enumerate(STAGES):
    parts.append(
        ...
        '<div style="background:#0d0720;border:1px solid ' + color + ';border-radius:6px;'
...
""", unsafe_allow_html=True)
    ...
    st.metric("CV F1 (macro)",  f"{train_meta['cv_f1_mean']:.3f} ± {train_meta['cv_f1_std']:.3f}")

...
monthly_fatal = (
    ...
    .select(["yr", "mo", pl.col("len").alias("fatal")])
)
fatal_rate = (
    monthly_total
    .join(monthly_fatal, on=["yr", "mo"], how="left")
    .with_columns(pl.col("fatal").fill_null(0))
    .with_columns(
        (pl.col("fatal") / pl.col("total") * 100).alias("fatal_pct")
    )
    .sort(["yr", "mo"])
    .with_columns(
        (pl.col("yr").cast(pl.Utf8) + "-" + pl.col("mo").cast(pl.Utf8).str.zfill(2)).alias("label")
    )
)
...
covid_peak_label = covid_years.loc[covid_years["fatal_pct"].idxmax(), "label"] if len(covid_years) > 0 else None

fig2 = go.Figure()

# COVID shaded band
fig2.add_vrect(
    x0="2020-01", x1="2021-12",
    fillcolor="#7c3aed", opacity=0.08,
...
fig2.add_trace(go.Scatter(
    x=fatal_pd["label"], y=fatal_pd["fatal_pct"],
    name="Monthly rate", mode="lines",
...
# Rolling average
fig2.add_trace(go.Scatter(
    x=fatal_pd["label"], y=fatal_pd["rolling_12"],
...
if covid_peak is not None and covid_peak_label is not None:
    fig2.add_annotation(
        x=covid_peak_label, y=covid_peak,
        text=f"Peak {covid_peak:.3f}%",
        showarrow=True, arrowhead=2, arrowcolor="#ef4444",
        font=dict(family="DM Mono", size=9, color="#ef4444"),
        bgcolor="#1a0808", bordercolor="#ef4444", borderwidth=1,
        ax=40, ay=-30,
    )
...
fig2.update_layout(
    LAYOUT, height=320, margin=dict(t=30, b=40, l=60, r=20),
    xaxis=dict(**XAXIS, tickangle=-45, nticks=20),
...
palette = [
    "#6b6880", "#6b6880", "#6b6880", "#6b6880", "#6b6880",
    "#6b6880", "#6b6880", "#7c3aed", "#a855f7", "#ef4444",
]
...
fig3.update_layout(
    LAYOUT, height=320, margin=dict(t=20, b=40, l=50, r=20),
    xaxis=dict(**XAXIS, tickvals=list(range(1,13)), ticktext=MONTH_NAMES, title="Month"),

...
import numpy as np
from main import (
    load_data, load_model, load_preprocessor,
    load_feature_meta, load_train_meta
)
...
<div style="padding:2rem 0 1.5rem 0;">
  ...
  <p style="color:#9d99ae;font-size:0.95rem;max-width:560px;line-height:1.75;margin-top:0.5rem;">
    Simulate crash conditions and see what the model predicts.
    Built on XGBoost trained exclusively on Queens collision data.
  </p>
</div>
...
with st.expander("Model Performance Summary", expanded=False):
    ...
    st.markdown(f"""
    <div style="font-family:'DM Mono',monospace;font-size:0.72rem;color:#6b6880;
                margin-top:0.8rem;line-height:1.8;">
      Trained on {train_meta['n_train']:,} rows · Tested on {train_meta['n_test']:,} rows ·
      CV F1: {train_meta['cv_f1_mean']:.3f} ± {train_meta['cv_f1_std']:.3f} ·
      Borough: {train_meta['borough']}
    </div>
    """, unsafe_allow_html=True)
...
if st.button("Predict Severity →"):
    ...
    tint = {"No Injury":"#0a1a0f","Minor Injury":"#1a1500","Major Injury":"#1a0e00","Fatal":"#1a0808"}

    st.markdown(f"""
    <div style="margin:1.5rem 0;padding:2rem;background:{tint[predicted_label]};
      ...
      <div style="font-family:'DM Mono',monospace;font-size:0.8rem;color:#6b6880;margin-top:0.5rem;">
        Confidence: {proba[predicted_idx]:.1%}
      </div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("#### Probability by Severity Class")
    ...
    xaxis = dict(**XAXIS, tickfont=dict(family="DM Sans",size=12))
    ...
    top_imp = sorted(train_meta["feature_importances"].items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    st.markdown("""
    <div style="font-family:'DM Mono',monospace;font-size:0.65rem;color:#2e2e42;
